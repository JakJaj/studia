...
/**********************************************************/
//tu wpisz funkcje
int pop_front(Wezel **pierwszy, int *zwracanaWartosc){
    if(*pierwszy == NULL){ //sprawdzanie czy lista jest pusta
        return -1;
    }

    Wezel *biezacy = *pierwszy;
    Wezel *doWyczyszczenia = *pierwszy;
    *zwracanaWartosc = biezacy->wartosc;
    
    biezacy = biezacy->nastepny; //drugi
    free(doWyczyszczenia);
